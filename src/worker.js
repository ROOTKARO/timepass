...
async function onUpdate(update, env) {
  ...
  if (text === "/start") {
    return send(env, chatId, "Thanks For Choosing RELAX TOOL -\n/search market_model=RMX5210 project_no=22667\n/search rmx5210 22667");
  }
  if (!text.toLowerCase().startsWith("/search")) return send(env, chatId, "Use /search");

  const rows = await loadRows(env);
  const query = text.replace(/^\/search\s*/i, "").toLowerCase().trim();
  if (!query) return send(env, chatId, "Usage: /search market_model=... project_no=...");
  ...
  const out = hits.slice(0, 20).map((r, i) =>
